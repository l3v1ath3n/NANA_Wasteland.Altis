...
if((player == driver _vehicle) && (!_eng))then {
	...
	if (!isNil "_price") then 
	{
		...
		{	
			...
			sleep 5;
			["Evaluating this vehicle...", 5] call mf_notify_client;
			sleep 5;
			["Carefully removing your current VIN tag.", 5] call mf_notify_client;
			sleep 5;
			["Installing new locks.", 5] call mf_notify_client;
			sleep 5;
			["Adding new VIN tag.", 5] call mf_notify_client;
			sleep 5;		
			["Taking another coffee break.", 5] call mf_notify_client;
			sleep 5;
			["Forging title papers.", 5] call mf_notify_client;
			sleep 5;
			...
			playSound "FD_Finish_F";
			_text = format ["%1 is now yours to lock and save. Fully repaired and refuelled.", _type];
