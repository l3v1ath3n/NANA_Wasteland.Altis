...
if (_eng) exitWith {
	//titleText ["Engine running. Resupply aborted!", "PLAIN DOWN", 0.5];
	vehicle player vehicleChat format['Engine running. Resupply aborted!'];
	mutexScriptInProgress = false;
};
//10 second start warning
_text = format ["Cost for service is $%1 for this vehicle. This will take some time.\nYou can always abort by getting out of the vehicle OR starting your engine.\nService begins in 10 seconds!", _price];
titleText [_text, "PLAIN DOWN", 0.5];
vehicle player vehicleChat format["Cost for service is $%1 for this vehicle. This will take some time. You can always abort by getting out of the vehicle OR starting your engine. Service begins in 10 seconds!", _price];
sleep 10;
...
//		if ({alive _x} count (_vehicle nearEntities [["O_Heli_Transport_04_covered_F", "O_Truck_03_device_F", "Land_Pod_Heli_Transport_04_repair_F"], RESUPPLY_TRUCK_DISTANCE]) == 0) then
//		{
...
player setVariable["cmoney",(player getVariable "cmoney")-_price,true];
	...
	vehicle player vehicleChat format["Your vehicle is ready!"];
	mutexScriptInProgress = false;
};
